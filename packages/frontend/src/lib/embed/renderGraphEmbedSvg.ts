...
export function renderGraphEmbedSvg(
  ...
  const stats = [
    ...
    { value: cost, label: "spent", color: palette.cost },
    {
      value: rank,
      label: `rank · ${sortBy}`,
...
<svg data-template="graph" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokscale contribution graph for @${escapeXml(data.user.username)}">
  ...
  <rect x="${PAD}" y="23" width="3" height="14" rx="1.5" fill="${palette.brand}"/>
  <text x="${PAD + 12}" y="35" fill="${palette.text}" font-size="15" font-weight="700" font-family="${FIGTREE_FONT_STACK}">@${escapeXml(data.user.username)}</text>
  ${stats
    .map((stat, index) => {
      const x = 398 + index * 88;
      return `<text x="${x}" y="24" fill="${palette.muted}" font-size="10" font-weight="600" font-family="${FIGTREE_FONT_STACK}">${escapeXml(stat.label)}</text>
  <text x="${x}" y="43" fill="${stat.color}" font-size="14" font-weight="700" font-family="${FIGTREE_FONT_STACK}">${escapeXml(stat.value)}</text>`;
    })
    .join("\n  ")}
  ${graph.svg}
  ${cardFooter({
