...
- Goals: Make a profile understandable in one viewport; foreground usage history; make period, metric, and provider controls obvious; retain sharing/embed utility; work cleanly from 320px through desktop.
...
- User jobs: Identify the person, understand scale and recency, inspect a time trend, filter by provider, compare token categories/models/devices, and share or embed the result.
- Key contexts of use: Desktop comparison, mobile link preview, keyboard-only navigation, and profiles with long names, many providers, sparse data, or no optional device/model detail.
